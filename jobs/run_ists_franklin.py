...
"""ISTS sub-project A2 run (Franklin OH). Mirrors run_ists_harris.

    python -m jobs.run_ists_franklin --dry-run   # print + metrics, no DB write
    python -m jobs.run_ists_franklin             # also upserts to ists_judgments
"""
...
# Allow running as a plain script (python jobs/run_ists_franklin.py) under the
# scheduler, not just `python -m jobs.run_ists_franklin`.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from scrapers.ohio.franklin_judgments import FranklinJudgmentScraper
from services.ists_prior_work import annotate_prior_work
from services import ists_store

logging.basicConfig(level=logging.INFO)
log = logging.getLogger("ists.franklin")
...
def _metrics(records) -> str:
    ...
    return (f"records={len(records)} | disposition-date buckets={dict(buckets)} | "
            f"prior_phone={prior_phone} | prior_called={prior_called}")


async def main(dry_run: bool) -> None:
    scraper = FranklinJudgmentScraper()
    records = scraper.scrape()  # sync (requests CSV download, no browser)
    ...
    if dry_run:
        for r in records[:25]:
            log.info("DRY %s | %s | %s | ddate=%s | disp=%s | prior_phone=%s",
                     r.case_number, r.defendant_name, r.property_address,
                     r.disposition_date, r.disposition_desc, r.prior_phone)
