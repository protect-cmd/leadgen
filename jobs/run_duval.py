...
"""
Duval County (Jacksonville) FL eviction scraper runner.

Usage:
    python -m jobs.run_duval                       # scraper-only, no DB write
    python -m jobs.run_duval --yes-write-supabase
    python -m jobs.run_duval --lookback-days 7
    python -m jobs.run_duval --dry-run
"""
...
class DuvalRunSummary:
    ...
    def to_lines(self) -> list[str]:
        ...
        return [
            "Duval County FL — Residential Eviction",
            f"Mode:                  {mode}",
...
async def main(
    *,
    ...
    log.info("Duval FL: starting run (lookback=%d days)", lookback_days)

    scraper = DuvalScraper(lookback_days=lookback_days, headless=headless)
    ...
    log.info(
        "Duval FL: %d total | %d with address | %d unknown address",
        len(filings), len(with_addr), len(without_addr),
    )

    if dry_run:
        log.info("Duval FL: dry-run — printing first 25 filings")
        for f in filings[:25]:
            log.info(
                "  %s | %s | %s | filed=%s",
                f.case_number, f.tenant_name, f.property_address, f.filing_date,
            )
        log.info("Duval FL: dry-run — %d filings NOT written", len(filings))
        return DuvalRunSummary(
    ...
    if yes_write_supabase and with_addr:
        ...
        for filing in with_addr:
            if await dedup_service.is_duplicate(filing.case_number):
                ...
                inserted += 1
        log.info(
            "Duval FL: Supabase push — %d inserted, %d duplicates",
            inserted, duplicates,
        )
        wrote = True

    summary = DuvalRunSummary(
        total_filings=len(filings),
        defendants_with_address=len(with_addr),
        defendants_unknown_address=len(without_addr),
        lookback_days=lookback_days,
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Run Duval County (Jacksonville) FL eviction scraper. "
