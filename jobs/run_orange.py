...
"""
Orange County FL eviction scraper runner.

Usage:
    python -m jobs.run_orange                       # scraper-only, no DB write
    python -m jobs.run_orange --yes-write-supabase
    python -m jobs.run_orange --lookback-days 14
    python -m jobs.run_orange --dry-run
"""
...
from scrapers.florida.orange import OrangeScraper
...
class OrangeRunSummary:
    ...
    def to_lines(self) -> list[str]:
        ...
        return [
            "Orange County FL — Residential Eviction",
            f"Mode:                  {mode}",
...
async def main(
    *,
    ...
    log.info("Orange FL: starting run (lookback=%d days)", lookback_days)

    scraper = OrangeScraper(
    ...
    log.info(
        "Orange FL: %d total | %d with address | %d unknown address",
        len(filings), len(with_addr), len(without_addr),
    )

    if dry_run:
        log.info("Orange FL: dry-run — printing first 25 filings")
        for f in filings[:25]:
            log.info(
                "  %s | %s | %s | filed=%s",
                f.case_number, f.tenant_name, f.property_address, f.filing_date,
            )
        log.info("Orange FL: dry-run — %d filings NOT written", len(filings))
        return OrangeRunSummary(
    ...
    if yes_write_supabase and with_addr:
        ...
        for filing in with_addr:
            if await dedup_service.is_duplicate(filing.case_number):
                ...
                inserted += 1
        log.info(
            "Orange FL: Supabase push — %d inserted, %d duplicates",
            inserted, duplicates,
        )
        wrote = True

    summary = OrangeRunSummary(
        total_filings=len(filings),
        defendants_with_address=len(with_addr),
        defendants_unknown_address=len(without_addr),
        lookback_days=lookback_days,
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        description=(
            "Run Orange County FL eviction scraper. "
