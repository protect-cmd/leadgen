"""
Import Hamilton County OH yellow-source proof contacts into Supabase.

Reads tmp/hamilton_yellow_enrichment_proof.csv, finds rows with a phone number,
...
async def run_import(csv_path: Path) -> ImportSummary:
    ...
    # Re-scrape Hamilton to get full Filing objects for these case numbers
    scraper = HamiltonCountyMunicipalScraper(lookback_days=14)
...
async def main_async(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Import Hamilton OH yellow-source proof contacts into Supabase."
    )
    parser.add_argument(
        "--csv",
        type=Path,
    ...
    print()
    print("Hamilton proof contact import")
