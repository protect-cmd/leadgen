...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # Ensure ONLY Civil is checked — uncheck Criminal, Family, Probate if ticked
        for box_id, label in [("chkDtRangeCriminal", "Criminal"),
                               ("chkDtRangeFamily",   "Family"),
                               ("chkDtRangeProbate",  "Probate")]:
            cb = search_frame.locator(f"#{box_id}")
            if await cb.is_checked():
                await cb.click()
                log.info(f"Unchecked {label}.")
